...
class RoutingService:
    def resolver_rutas(
        ...
        repartidores: list[CapacidadRepartidor],
    ) -> OptimizacionResponse:
        logger.info(
            f"Iniciando optimización con {len(puntos_entrega)} puntos y {len(repartidores)} repartidores"
        )
        ...
        if not puntos_entrega:
            ...
            raise ValueError(
                "No se proporcionaron puntos de entrega. Por favor, añade al menos un punto de entrega."
            )

        if not repartidores:
            logger.warning("No hay repartidores")
            raise ValueError(
                "No hay repartidores disponibles. Por favor, asegúrate de que hay repartidores disponibles hoy."
            )

        capacidad_total = sum(r.capacidad for r in repartidores)
        peso_total = sum(p.peso for p in puntos_entrega)
        logger.info(
            f"Capacidad total disponible: {capacidad_total}, Peso total a repartir: {peso_total}"
        )

        if peso_total > capacidad_total:
            logger.error(
                f"ERROR: No hay suficiente capacidad. Necesitas {peso_total} pero solo tienes {capacidad_total}"
            )
            raise ValueError(
        ...
        modelo.AddDimensionWithVehicleCapacity(
            capacidad_callback, 0, [int(v.capacidad) for v in repartidores], True, "Capacidad"
        )
        ...
        if solucion:
            ...
            return self.__extraer_rutas(
                solucion, modelo, gerente, repartidores, puntos_entrega, matriz
            )
        else:
    ...
    def __validar_matriz(self, matriz: list[list[float]], cantidad_puntos: int) -> None:
        ...
        if not matriz or len(matriz) != tamano_esperado:
            raise ValueError(
                "No se pudo construir una matriz de distancias válida para los puntos seleccionados."
            )

        for fila in matriz:
            if len(fila) != tamano_esperado:
                raise ValueError(
                    "La matriz de distancias tiene un tamaño inconsistente para los puntos seleccionados."
                )
        ...
        for nodo in range(1, tamano_esperado):
            ...
            conexiones_validas = [
                distancia
                for indice, distancia in enumerate(fila)
                if indice != nodo and distancia is not None
            ]
        ...
        if any(
            matriz[origen][destino] is None
            for origen in range(tamano_esperado)
            for destino in range(tamano_esperado)
            if origen != destino
        ):
            raise ValueError(
    ...
    def __extraer_rutas(
        ...
        for i, repartidor in enumerate(repartidores):
            ...
            if paradas:
                ...
                logger.info(
                    f"Repartidor {repartidor.id}: {len(paradas)} paradas, distancia: {distancia}m, tiempo: {tiempo_estimado}s"
                )
                rutas.append(
                    RutaRepartidor(
    ...
    def __calcular_distancia_ruta(
        self, paradas: list[int], puntos: list[PuntoEntrega], matriz: list[list[float]]
    ) -> float:
