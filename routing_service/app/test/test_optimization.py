...
class TestOptimizationService:
    ...
    @pytest.mark.asyncio
    async def test_optimizar_llamadas_correctas(
        self, request_valido, matriz_distancias_mock, matriz_tiempos_mock
    ):
        """Test que verifica que el servicio llama a los métodos correctos"""
        service = OptimizationService()

        # Mock de los servicios internos
        service.matrix_service.get_matriz_distancias = AsyncMock(
            return_value=matriz_distancias_mock
        )
        ...
        service.routing_service.resolver_rutas = MagicMock(
            return_value=OptimizacionResponse(
                rutas=[
                    RutaRepartidor(
                        repartidor_id=1,
                        ruta=[1, 2],
    ...
    @pytest.mark.asyncio
    async def test_optimizar_calcula_tiempo_correcto(
        self, request_valido, matriz_distancias_mock, matriz_tiempos_mock
    ):
        ...
        service.geometry_service.get_geometry_route = AsyncMock(return_value=[])
    ...
    def test_calcular_tiempo_valido(self, request_valido):
        ...
        matriz_tiempos = [[0, 60, 90], [60, 0, 50], [90, 50, 0]]
...
class TestCalcularTiempoEdgeCases:
    ...
    def test_calcular_tiempo_un_punto(self):
        ...
        matriz_tiempos = [[0, 60], [60, 0]]

        tiempo = service._OptimizationService__calcular_tiempo(
            paradas=[1], puntos_entrega=puntos, matriz_tiempos=matriz_tiempos
        )
    ...
    def test_calcular_tiempo_varios_puntos(self):
        ...
        matriz_tiempos = [[0, 60, 90, 100], [60, 0, 50, 70], [90, 50, 0, 40], [100, 70, 40, 0]]

        # Ruta: 1 -> 2 -> 3
        tiempo = service._OptimizationService__calcular_tiempo(
            paradas=[1, 2, 3], puntos_entrega=puntos, matriz_tiempos=matriz_tiempos
        )
...
class TestPuntosEntregaModels:
    ...
    def test_crear_punto_entrega_valido(self):
        ...
        punto = PuntoEntrega(id=1, latitud=4.7200, longitud=-74.0800, peso=5)
...
class TestCapacidadRepartidorModel:
    ...
    def test_crear_capacidad_con_alias(self):
        ...
        capacidad = CapacidadRepartidor(idRepartidor=1, capacidadRepartidor=100)
    ...
    def test_crear_capacidad_sin_alias(self):
        ...
        capacidad = CapacidadRepartidor(id=2, capacidad=50)
